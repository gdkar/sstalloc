...
/* Mappings */
ST_CACHE_ALIGNED( int cls2size[128]);
char sizemap[256];
char sizemap2[128];

/* Private metadata */
ST_THREAD_LOCAL init_state thread_state = UNINITIALIZED;
ST_THREAD_LOCAL lheap_t *local_heap = NULL;
...
static void* gpool_gc(void* arg)
{
    ...
    while(1) {
        ...
        ptr = (char*) stack_pop_tagged(&global_pool.free_dc_head[get_core_id()]);
        if(ptr) {
            void *ptr_end = ST_PAGE_ROUND_DOWN(ptr + ST_CHUNK_SIZE);
            void *ptr_start = ST_PAGE_ROUND_UP(ptr);
            madvise(ptr_start, (uintptr_t)ptr_end - (uintptr_t)ptr_start, MADV_DONTNEED); 
            stack_put_tagged(&global_pool.released_dc_head[get_core_id()], ptr);
        }
    }
}
#endif
...
{
    global_pool.pool_start = ST_RAW_POOL_START;
    global_pool.pool_end =   ST_RAW_POOL_START;
    global_pool.free_start = ST_RAW_POOL_START;
...
inline static chunk_t *gpool_acquire_chunk()
{
    ...
    ptr = stack_pop_tagged(&global_pool.free_dc_head[get_core_id()]);
    if (ptr) {
        return (chunk_t *) ptr;
    }

#ifdef RETURN_MEMORY
    ptr = stack_pop_tagged(&global_pool.released_dc_head[get_core_id()]);
    if (ptr) {
        // XXX: Fix me
        ((chunk_t *) ptr)->numa_node = get_core_id();
        touch_memory_range(ptr, ST_CHUNK_SIZE);
        return (chunk_t *) ptr;
    }
#endif
...
inline static void gpool_release_chunk(dchunk_t *dc)
{
    stack_push_tagged(&global_pool.free_dc_head[dc->numa_node], dc);
}
...
static lheap_t *gpool_acquire_lheap()
{
    ...
    lh = stack_pop_tagged(&global_pool.free_lh_head[get_core_id()]);
...
inline static void lheap_init(lheap_t * lh)
{
    ...
    int size_cls;
    lh->dummy_chunk.size_cls = ST_DUMMY_CLASS;
    lh->dummy_chunk.free_blk_cnt = 1;

    for (size_cls = 0; size_cls < ST_MAX_BLOCK_CLASS; size_cls++) {
...
inline static void lheap_replace_foreground
    ...
    dc = (dchunk_t *) lh->background[size_cls].next;
    if (dc != NULL) {
        list_remove(dc, &lh->background[size_cls]);
        goto finish;
    }
    ...
    dc->owner = lh;
    fast_queue_init((fast_stack*) & (dc->remote_free_head));
...
inline static void dchunk_change_cls(dchunk_t * dc, int size_cls)
{
    ...
    int data_offset = DCH;
    dc->blk_cnt = (ST_CHUNK_SIZE - data_offset) / size;
...
inline static void dchunk_collect_garbage(dchunk_t * dc)
{
    *(ptr_t**)&(dc->free_head) = (ptr_t)counted_chain_dequeue(&dc->remote_free_head, &dc->free_blk_cnt);
}
...
inline static void *dchunk_alloc_obj(dchunk_t * dc)
{
    ...
    ret = (void*)(dc->free_head);

    if (unlikely(!ret)) {
        ret = dc->free_mem;
        dc->free_mem += dc->block_size;
    } else {
        dc->free_head = (seq_stack_t*)ret;
    }
...
inline static dchunk_t *dchunk_extract(void *ptr)
{
    return (dchunk_t *) ((uintptr_t)ptr - ((uintptr_t)ptr % ST_CHUNK_SIZE));
}
...
inline static void obj_buf_flush(obj_buf_t * bbuf)
{
    ...
    bbuf->dc = NULL;
    bbuf->first = NULL;
    bbuf->free_head =(seq_stack_t*)NULL;
...
inline static void *large_malloc(size_t size)
{
    size_t alloc_size = ST_PAGE_ROUND_UP(size + ST_CHUNK_SIZE);
    void *mem = page_alloc(NULL, alloc_size);
    void *mem_start = (char*)mem + ST_CHUNK_SIZE - ST_CACHE_LINE;
...
inline static void *small_malloc(int size_cls)
{
  ...
  retry:
    ...
    if (unlikely(--dc->free_blk_cnt == 0)) {
        ...
        if (unlikely(dc->size_cls == ST_DUMMY_CLASS)) {
            /* A dummy chunk */
            dc->free_blk_cnt = 1;
            goto retry;
        }
...
inline static void local_free(lheap_t * lh, dchunk_t * dc, void *ptr)
{
    ...
    switch (dc->state) {
    case FULL:
        list_insert_front(dc, &lh->background[dc->size_cls]);
        dc->state = BACKGROUND;
        break;
    case BACKGROUND:
        if (unlikely(free_blk_cnt == dc->blk_cnt)) {
            int free_cnt = lh->free_cnt;
            list_remove(dc, &lh->background[dc->size_cls]);
            if (free_cnt >= ST_MAX_FREE_CHUNK) {
...
ST_THREAD_LOCAL int buf_cnt;
inline static void remote_free(lheap_t * lh, dchunk_t * dc, void *ptr)
{
    /* Put the object in a local buffer rather than return it to owner */
    int tag = ((unsigned long long)dc / ST_CHUNK_SIZE) % ST_BLOCK_BUF_CNT;
...
static void *large_memalign(size_t boundary, size_t size) {
    ...
    size_t padding = boundary + ST_CHUNK_SIZE;
    size_t alloc_size = ST_PAGE_ROUND_UP(size + padding);
    void *mem = page_alloc(NULL, alloc_size);
...
void *malloc(size_t size)
{
    ...
    if (likely(size_cls < ST_MAX_BLOCK_CLASS)) {
...
void free(void *ptr)
{
    ...
    } else if(likely(target_lh != ST_LARGE_OWNER)){
        check_init();
        lh = local_heap;
...
void *realloc(void* ptr, size_t size)
{
    ...
    if (dc->owner != ST_LARGE_OWNER) {
        ...
        /* Try to do mremap */
        int new_size = ST_PAGE_ROUND_UP(size + ST_CHUNK_SIZE);
...
void *valloc(size_t size)
{return memalign(ST_PAGE_SIZE, size);}
void *pvalloc(size_t size)
{return memalign(ST_PAGE_SIZE,ST_PAGE_ROUND_UP(size)); }
